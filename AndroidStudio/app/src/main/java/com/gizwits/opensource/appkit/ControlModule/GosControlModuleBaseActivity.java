...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 * ===========================================================
	 */
	// 数据点"Examination"对应的标识名
	protected static final String KEY_EXAMINATION = "Examination";
	// 数据点"Deferred"对应的标识名
	protected static final String KEY_DEFERRED = "Deferred";
	// 数据点"Classroom"对应的标识名
	protected static final String KEY_CLASSROOM = "Classroom";
	// 数据点"Lecture"对应的标识名
	protected static final String KEY_LECTURE = "Lecture";
	// 数据点"Students"对应的标识名
	protected static final String KEY_STUDENTS = "Students";
	 ...
	 * ===========================================================
	 */
	// 数据点"Examination"对应的存储数据
	protected static boolean data_Examination;
	// 数据点"Deferred"对应的存储数据
	protected static boolean data_Deferred;
	// 数据点"Classroom"对应的存储数据
	protected static int data_Classroom;
	// 数据点"Lecture"对应的存储数据
	protected static int data_Lecture;
	// 数据点"Students"对应的存储数据
	protected static int data_Students;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				if (dataKey.equals(KEY_EXAMINATION)) {
					data_Examination = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_DEFERRED)) {
					data_Deferred = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_CLASSROOM)) {
					data_Classroom = (Integer) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_LECTURE)) {
					data_Lecture = (Integer) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_STUDENTS)) {
			
					data_Students = (Integer) map.get(dataKey);
				}
			}
		}
