...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagExamination)
    {
        info->event[info->num] = EVENT_Examination;
        info->num++;
        dataPoints->valueExamination = gizStandardDecompressionValue(Examination_BYTEOFFSET,Examination_BITOFFSET,Examination_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagDeferred)
    {
        info->event[info->num] = EVENT_Deferred;
        info->num++;
        dataPoints->valueDeferred = gizStandardDecompressionValue(Deferred_BYTEOFFSET,Deferred_BITOFFSET,Deferred_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagClassroom)
    {
        info->event[info->num] = EVENT_Classroom;
        info->num++;
        dataPoints->valueClassroom = gizStandardDecompressionValue(Classroom_BYTEOFFSET,Classroom_BITOFFSET,Classroom_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagLecture)
    {
        info->event[info->num] = EVENT_Lecture;
        info->num++;
        dataPoints->valueLecture = gizStandardDecompressionValue(Lecture_BYTEOFFSET,Lecture_BITOFFSET,Lecture_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueExamination != cur->valueExamination)
    {
        GIZWITS_LOG("valueExamination Changed\n");
        ret = 1;
    }
    if(last->valueDeferred != cur->valueDeferred)
    {
        GIZWITS_LOG("valueDeferred Changed\n");
        ret = 1;
    }
    if(last->valueClassroom != cur->valueClassroom)
    {
        GIZWITS_LOG("valueClassroom Changed\n");
        ret = 1;
    }
    if(last->valueLecture != cur->valueLecture)
    {
        GIZWITS_LOG("valueLecture Changed\n");
        ret = 1;
    }

    if(last->valueStudents != cur->valueStudents)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueStudents Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Examination_BYTEOFFSET,Examination_BITOFFSET,Examination_LEN,(uint8_t *)devStatusPtr,dataPoints->valueExamination);
    gizStandardCompressValue(Deferred_BYTEOFFSET,Deferred_BITOFFSET,Deferred_LEN,(uint8_t *)devStatusPtr,dataPoints->valueDeferred);
    if(dataPoints->valueClassroom >= Classroom_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueClassroom Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(Classroom_BYTEOFFSET,Classroom_BITOFFSET,Classroom_LEN,(uint8_t *)devStatusPtr,dataPoints->valueClassroom);  
    }
    if(dataPoints->valueLecture >= Lecture_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueLecture Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(Lecture_BYTEOFFSET,Lecture_BITOFFSET,Lecture_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLecture);  
    }
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));


    devStatusPtr->valueStudents = exchangeBytes(gizY2X(Students_RATIO,  Students_ADDITION, dataPoints->valueStudents)); 
